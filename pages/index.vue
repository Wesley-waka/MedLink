<template>
  ...
  <div>
        ...
        <Card v-for="(service, index) in services" :key="index" class="shadow-lg rounded-lg overflow-hidden bg-white">
          <template #header>
            <div class="relative">
              ...
              <h3
                class="absolute bottom-0 left-0 text-xl font-semibold text-white bg-black bg-opacity-60 p-3 rounded-t-lg">
                {{ service.title }}
              </h3>
            </div>
          </template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-8 bg-white rounded-md">
      <div class="flex items-center justify-center">
        <img src="https://images.pexels.com/photos/305565/pexels-photo-305565.jpeg" alt="Feature Image"
          class="w-full h-auto rounded-md" />
      </div>
      <div class="flex items-center p-4">
        <p class="text-gray-800">
          Guaranteeing the attention, security, and trust of the patients, respecting their life regardless of their
          national origin.
          Additionally, assuring ethical and honest practices, respecting all the existing regulations in each country
          where we have a presence through teamwork,
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="testimonial in testimonials" :key="testimonial.name"
          ...
          <img :src="testimonial.image" alt="Testimonial image"
            class="w-24 h-24 rounded-full object-cover mb-4 border-2 border-gray-200" />
      ...
      <form @submit.prevent="handleSubmit" class="max-w-md mx-auto bg-white p-8 rounded-lg shadow-md">
        <div class="mb-4">
          ...
          <input type="text" id="name" v-model="form.name" required
            class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Your Name" />
        </div>
        <div class="mb-4">
          <label for="email" class="block text-gray-700 text-sm font-bold mb-2">Email</label>
          <input type="email" id="email" v-model="form.email" required
            class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Your Email" />
        </div>
        <div class="mb-4">
          <label for="message" class="block text-gray-700 text-sm font-bold mb-2">Message</label>
          <textarea id="message" v-model="form.message" required rows="4"
            class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Your Message"></textarea>
        </div>
